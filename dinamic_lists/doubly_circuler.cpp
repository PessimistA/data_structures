...
void addition(node_pointer* head1, node_pointer* head2, node_pointer* head3) {
    node_pointer p1 = *head1;
    node_pointer p2 = *head2;
    int p1_done = 0, p2_done = 0; // Döngü bitişlerini takip etmek için

    while (!p1_done || !p2_done) {//bunların ikisi de 1 olduğunda 0 olurlar ve sonuç 0 olur yani durur
        if (!p1_done && (p2_done || p1->exponen > p2->exponen)) {//p1_done hala sıfır olmalı
            // p1'in üssü büyükse veya p2 bitmişse
            insert_item(head3, p1->katsayı, p1->exponen);
            p1 = p1->rightlink;
        ...
        else if (!p2_done && (p1_done || p1->exponen < p2->exponen)) {
            //p2_done hala sıfır olmalı
            // p2'nin üssü büyükse veya p1 bitmişse
            insert_item(head3, p2->katsayı, p2->exponen);
            p2 = p2->rightlink;
        ...
        else if (!p1_done && !p2_done && p1->exponen == p2->exponen) {
            // Aynı üsse sahip terimler
            insert_item(head3, p1->katsayı + p2->katsayı, p1->exponen);
            p1 = p1->rightlink;
            p2 = p2->rightlink;
