...
void insert(int x) {
    ...
    hole = ++the_size;
    for (; x > array[hole / 2] && hole > 1; hole /= 2) {  // Max-heap için karşılaştırma
        array[hole] = array[hole / 2];  // Ebeveyn elemanını aşağı kaydırıyoruz.
    }
    array[hole] = x;
}
void insert_recursive(int x, int hole) {
    if (!(x > array[hole / 2]&& hole>1))
    {
        array[hole] = x;
        return;
    }
...
void recursive_percolateDown(int hole, int tmp) {// bu hala hatalı
    if (!(hole < the_size / 2))
    {
        array[hole] = tmp;
        return;
